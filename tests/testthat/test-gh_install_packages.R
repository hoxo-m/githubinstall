...
if (!("lubridate" %in% installed.packages(lib.loc = tmp)[, "Package"])) {
  install.packages(c("lubridate","bsts"), lib = tmp, repos = "https://cran.rstudio.com/")
}

test_that("Install a single package", {
  repo <- "AnomalyDetection"

  act <- suppressWarnings(gh_install_packages(repo, ask = FALSE, lib = tmp, destdir = tmp))
